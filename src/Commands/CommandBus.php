...
class CommandBus
{
    ...
    public function addCommand($command)
    {
        ...
        if ($command instanceof CommandInterface) {
            ...
            if (! is_null($command->getLongName())) {
                $this->commands[$command->getLongName()] = $command;
            }
    ...
    public function handler($message, Update $update)
    {
        ...
        if (!empty($match) || array_key_exists($message, $this->commands)) {
            $command = $match[1] ?? $message;
            $arguments = $match[3] ?? null;
